    ...
    while (True):
        ...
        if (tree_fill_nodes[trigger_node_index].leaf_counter == leaf_threshold):            # 리프 노드 카운터 값이 threshold에 이르면
            ...
            if not tree_fill_nodes[dt_node_index].left_full:
                ...
                sender = tree_fill_nodes[dt_node_index]
                for d in range(detect_tree_depth - 1, -1, -1):                              # 내부 노드 트리 순회
                    direction = dt_node_index % 2                                           # direction이 0이면 왼쪽, 1이면 오른쪽
                    dt_node_index = dt_node_index // 2                                      # FULL 메시지를 받을 부모 노드
                    receiver = tree_fill_nodes[dt_node_index]
                    message_count = message_sr(sender, receiver, message_count)             # FULL 메시지 전달
                    if direction == 0 and tree_fill_nodes[dt_node_index].left_full:         ####
                        print("Error!!!")
                    if direction == 1 and tree_fill_nodes[dt_node_index].right_full:        ####
                        print("Error!!!")
                    if direction == 0:
                        tree_fill_nodes[dt_node_index].left_full = True
                    else:
                ...
                sender = tree_fill_nodes[dt_node_index]
                for d in range(detect_tree_depth - 2, -1, -1):                              # 내부 노드 트리 순회
                    direction = dt_node_index % 2                                           # direction이 0이면 왼쪽, 1이면 오른쪽
                    dt_node_index = dt_node_index // 2                                      # 코인을 받을 부모 노드
                    receiver = tree_fill_nodes[dt_node_index]
                    message_count = message_sr(sender, receiver, message_count)             # 코인 메시지 전달
            ...
            for d in range(detect_tree_depth - 1, -1, -1):                                  # 트리를 리프 노드부터 루트 노드로 순회
                coin_node_index = random.randrange(0, pow(2, d))                            # 코인 추가 노드 결정
                receiver = tree_fill_nodes[(pow(2, d) - 1) + coin_node_index]
                message_count = message_sr(sender, receiver, message_count)                 # 코인 메시지 전달
                if not tree_fill_nodes[(pow(2, d) - 1) + coin_node_index].coin_received:    # 코인을 전달할 노드에 이미 코인이 있는지 확인
                    tree_fill_nodes[(pow(2, d) - 1) + coin_node_index].coin_received = True # 코인이 없을 경우 코인 추가
                    break
                sender = tree_fill_nodes[(pow(2, d) - 1) + coin_node_index]                 # 코인이 있을 경우 상위 layer로 코인 메시지 전송
            if (tree_fill_nodes[0].coin_received):                                          # end-of-round 프로시저 시작 조건
